@php
    ...
    if (Auth::check()) {
        ...
        if (Auth::user()->employe && Auth::user()->employe->equipe_id) {
    ...
    $notificationCount = $invalidationNotifications->count() + $userNotifications->whereNull('read_at')->count();
@endphp
<!DOCTYPE html>
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}" class="h-full">
    ...
    @php
        $isOperatorInterface = Auth::check() && Auth::user()->isOperateur();
    @endphp

    <body class="h-full text-slate-800 antialiased"
          x-data="{
              sidebarOpen: false,
              sidebarVisible: {{ $isOperatorInterface ? 'false' : 'true' }},
            ...
            <div class="flex items-center gap-1 sm:gap-3">
                @if(Auth::check())
                    <div x-data="{ isOpen: false }" class="relative inline-block">
                        <button @click="isOpen = !isOpen" class="p-1.5 rounded-lg text-slate-500 hover:text-slate-900 hover:bg-slate-100 transition-colors relative" title="Notifications">
                        ...
                        <div x-show="isOpen" @click.outside="isOpen = false" x-transition class="absolute right-0 mt-2 w-80 sm:w-96 bg-white rounded-2xl border border-slate-200 shadow-2xl z-50 p-4 text-slate-800" style="display: none;">
                            <h4 class="font-bold text-sm border-b border-slate-100 pb-2 mb-3 flex items-center justify-between">
                                <span>Notifications</span>
                                <span class="px-2 py-0.5 bg-emerald-100 text-emerald-700 rounded-full text-[10px] font-bold">{{ $notificationCount }}</span>
                            </h4>
                            <div class="max-h-80 overflow-y-auto space-y-3">
                                @if($invalidationNotifications->count() > 0)
                                    <div>
                                        <p class="text-[10px] uppercase tracking-wide font-semibold text-rose-600 mb-2">Tâches invalidées</p>
                                        @foreach($invalidationNotifications as $notif)
                                            <div class="p-2.5 rounded-xl bg-rose-50 border border-rose-100 text-xs text-left mb-2">
                                ...
                                @if($userNotifications->count() > 0)
                                    <div>
                                        <p class="text-[10px] uppercase tracking-wide font-semibold text-slate-500 mb-2">Autres notifications</p>
                                        @foreach($userNotifications as $notification)
                                            <a href="{{ data_get($notification->data, 'url', '#') }}" class="block p-2.5 rounded-xl {{ $notification->read_at ? 'bg-slate-50 border border-slate-100' : 'bg-emerald-50 border border-emerald-100' }} text-xs text-left mb-2">
                                                <p class="font-bold text-slate-900">{{ data_get($notification->data, 'title', 'Notification') }}</p>
                                                <p class="text-slate-600 mt-1">{{ data_get($notification->data, 'message', '') }}</p>
                                ...
                                @if($invalidationNotifications->count() === 0 && $userNotifications->count() === 0)
                                    <p class="text-center py-4 text-slate-400">Aucune notification pour le moment.</p>
                                @endif
                            </div>
                        </div>
                    </div>
        ...
        <div class="min-h-screen flex pt-14">
            ...
            <!-- ── Contenu principal ── -->
            <div class="flex-1 flex flex-col min-w-0 transition-all duration-300 ease-in-out" :class="sidebarVisible ? (sidebarCollapsed ? 'md:ml-20' : 'md:ml-64') : ''">
                <!-- Header de page -->
                @isset($header)
